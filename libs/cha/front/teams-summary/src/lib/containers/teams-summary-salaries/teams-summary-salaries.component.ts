...
export class TeamsSummarySalariesComponent implements OnInit {
  ...
  salaryHeaders = ['Name', 2026, 2027, 2028, 2029].map((x, i) => ({
    key: x,
    index: i,
  }));

  mobileSalaryHeaders = ['Name', 2026, 2027].map((x, i) => ({
    key: x,
    index: i,
  }));
      ...
      .subscribe((salaries: SalariesAndRatingsDto[]) => {
        this.forwardSalaries = salaries.map(
          (salary: SalariesAndRatingsDto) => ({
            ...
            2029: salary.salaries.season_2029,
          })
        );
      });
    ...
    this.defenseSalaries$
      ...
      .subscribe((salaries: SalariesAndRatingsDto[]) => {
        this.defenseSalaries = salaries.map(
          (salary: SalariesAndRatingsDto) => ({
            Name: `${salary.player_id.firstname} ${salary.player_id.lastname}`,
            2026: salary.salaries.season_2025,
            2027: salary.salaries.season_2026,
            2028: salary.salaries.season_2027,
            2029: salary.salaries.season_2028,
          })
        );
      });
